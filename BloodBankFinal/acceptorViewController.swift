...
class acceptorViewController: UIViewController, UITableViewDelegate, UITableViewDataSource,UIPickerViewDataSource,UIPickerViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "acceptorCell") as! acceptorTableViewCell
    ...
    override func viewDidLoad() {
        ...
        ref.child("user").child("acceptor").observe(.childAdded) {(snapshot) in
        ...
        ref.child("user").child("acceptor").observe(.childRemoved) {(snapshot) in
            let autoID = snapshot.key
            self.data[autoID] = nil
            self.filter()
            self.tableView.reloadData()
        }
        ref.child("user").child("acceptor").observe(.childChanged) {(snapshot) in
            let autoID = snapshot.key
            let obj = snapshot.value as! [String:Any]
            let name = obj["name"] as! String
            let email = obj["email"] as! String
